...
const ContentRow = ({ title, data, ranked = false }) => {
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8 relative group z-20">
      <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 flex items-center">
        <span className="w-1.5 h-8 bg-brand-red mr-4 rounded-full"></span>
        {title}
      </h2>
      ...
      <div className="relative">
        ...
        {/* Cards Container */}
        {ranked ? (
          <div className="flex flex-nowrap overflow-x-visible gap-4 pb-8 px-1 scroll-smooth">
            {data.map((item, index) => (
              <div key={item.id} className="relative min-w-[300px] h-[180px] flex items-center group cursor-pointer">
                <span className="text-[12rem] font-bold text-transparent leading-none -mb-8 z-0 translate-y-2"
                  style={{ WebkitTextStroke: '4px #595959' }}>
                  {index + 1}
                </span>
                <div className="absolute left-[70px] top-0 bottom-0 right-0 z-10 rounded-lg overflow-hidden border border-white/10 shadow-lg transition-transform duration-300 group-hover:scale-105 group-hover:border-white/30">
                  <img
                    src={item.image}
                    alt={item.title}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 pb-8 px-1">
            {data.map((item) => (
              <div
                key={item.id}
                className="relative w-full aspect-[2/3] rounded-xl overflow-hidden transition-all duration-500 hover:scale-105 hover:z-30 hover:shadow-[0_10px_40px_rgba(0,0,0,0.8)] cursor-pointer group/card border border-white/5 bg-rich-gray"
              >
                <img
                  src={item.image}
                  alt={item.title}
